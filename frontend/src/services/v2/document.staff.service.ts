...
export async function getPending(params?: {
  ...
  const response = await api.get<V2PaginatedResponse<V2Document>>(`${BASE_PATH}/pending`, { params })
  return response.data
}
...
export async function getExpiring(params?: {
  ...
  const response = await api.get<V2PaginatedResponse<V2Document>>(`${BASE_PATH}/expiring`, { params })
  return response.data
}
 ...
 */
export async function approve(id: string, notes?: string): Promise<V2ApiResponse<V2Document>> {
  const response = await api.post<V2ApiResponse<V2Document>>(
 ...
 * Rate limited: 60 requests per minute.
 */
export async function reject(id: string, reason: string): Promise<V2ApiResponse<V2Document>> {
  const response = await api.post<V2ApiResponse<V2Document>>(`${BASE_PATH}/${id}/reject`, { reason })
  return response.data
}
...
export async function setOcrData(
  ...
  const response = await api.post<V2ApiResponse<V2Document>>(`${BASE_PATH}/${id}/ocr`, {
    ocr_data: ocrData,
    confidence,
  })
  return response.data
...
export async function getPersonDocuments(
  ...
  const response = await api.get<V2ApiResponse<V2Document[]>>(
