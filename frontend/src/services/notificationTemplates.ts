...
export const notificationTemplatesApi = {
  ...
  async getAll(params?: { event?: string; channel?: string; is_active?: boolean }) {
    const response = await api.get<{ templates: NotificationTemplate[] }>(
  ...
  async getById(id: string) {
    const response = await api.get<{ template: NotificationTemplate }>(
  ...
  async create(data: CreateTemplateData) {
    const response = await api.post<{ template: NotificationTemplate }>(
      '/v2/staff/notification-templates',
      data
    )
    return response.data.template
  ...
  async update(id: string, data: Partial<CreateTemplateData>) {
    const response = await api.put<{ template: NotificationTemplate }>(
      `/v2/staff/notification-templates/${id}`,
      data
    )
    return response.data.template
   ...
   */
  async getConfig() {
    const response = await api.get<TemplateConfig>('/v2/staff/notification-templates/config')
  ...
  async testRender(data: TestRenderData) {
    const response = await api.post<TestRenderResponse>(
