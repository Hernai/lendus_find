...
const loadPreferences = async () => {
  ...
  error.value = null
  try {
    preferences.value = await notificationPreferencesApi.get('applicant')
  } catch (err: any) {
...
const loadEvents = async () => {
  try {
    ...
    // Filter events that are relevant for applicants
    events.value = config.events.filter((e) => {
      const applicantEvents = [
        'otp.sent',
      ...
      return applicantEvents.includes(e.value)
    })
  } catch (err) {
      ...
      <div v-else class="space-y-6">
          ...
          <div class="space-y-4">
            ...
            <label class="flex items-start gap-3 cursor-pointer">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-600">
                  Ver notificaciones dentro de la aplicación
                </p>
              </div>
            </label>
          </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <p class="text-sm text-gray-600 mb-4">
            Selecciona qué eventos deseas recibir. Las notificaciones de seguridad no se pueden
            desactivar.
          </p>
          <div class="space-y-3">
            <div
            ...
            >
              <input
                ...
                :disabled="event.value === 'otp.sent'"
                class="mt-1 w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
