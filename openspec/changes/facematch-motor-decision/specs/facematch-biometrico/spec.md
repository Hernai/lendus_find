...
El sistema DEBE (MUST) registrar SIEMPRE el resultado del facematch como verificación `face_match`: `is_verified = true` cuando el score alcanza el umbral (coincide) y `is_verified = false` cuando no lo alcanza (no coincide), guardando el score en la metadata. Una falla técnica del proveedor (error o timeout) NO DEBE registrar coincidencia ni no-coincidencia; el resultado queda sin concluir.

#### Scenario: La selfie coincide con la INE
- **WHEN** el facematch devuelve un score mayor o igual al umbral
- **THEN** se registra la verificación `face_match` con `is_verified = true` y el score en la metadata

#### Scenario: La selfie NO coincide con la INE
- **WHEN** el facematch devuelve un score menor al umbral
- **THEN** se registra la verificación `face_match` con `is_verified = false` y el score en la metadata

#### Scenario: Falla técnica del proveedor
- **WHEN** el facematch falla por error o timeout del proveedor
- **THEN** no se registra coincidencia ni no-coincidencia y el resultado queda sin concluir
