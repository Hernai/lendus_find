...
class AuthController extends Controller
{
        ...
        try {
            ...
            if ($channel === 'SMS' || $channel === 'WHATSAPP') {
                try {
                    ...
                    if ($sent) {
                        // Mark OTP as sent with provider info
                        $otp->markAsSent($result['message_id'] ?? null, 'sent');
                    } else {
    ...
    public function verifyOtp(Request $request): JsonResponse
    {
        ...
        // Check if OTP was actually sent via provider (not just generated)
        $otpWasSent = $otp->wasSent();
        ...
        if (!$user) {
            ...
            $user = User::create([
                ...
                'type' => 'APPLICANT',
                'is_active' => true,
                // Only mark as verified if OTP was actually sent
                'phone_verified_at' => ($request->phone && $otpWasSent) ? now() : null,
                'email_verified_at' => ($request->email && $otpWasSent) ? now() : null,
            ]);
        } else {
            // Update verification timestamps for existing user only if OTP was sent
            if ($request->phone && $otpWasSent && !$user->phone_verified_at) {
                $user->phone_verified_at = now();
                $user->save();
            }
            if ($request->email && $otpWasSent && !$user->email_verified_at) {
                $user->email_verified_at = now();
                $user->save();
            }
        }

        // Verify and lock phone/email via OTP on User ONLY if OTP was actually sent
        if ($otpWasSent) {
            if ($request->phone) {
                $this->verificationService->verify($user, 'phone', $request->phone, \App\Enums\VerificationMethod::OTP);
            }
            if ($request->email) {
                $this->verificationService->verify($user, 'email', $request->email, \App\Enums\VerificationMethod::OTP);
            }

            // If applicant already exists, also verify directly on applicant
            if ($user->applicant) {
                if ($request->phone) {
                    $this->verificationService->verify($user->applicant, 'phone', $request->phone, \App\Enums\VerificationMethod::OTP);
                }
                if ($request->email) {
                    $this->verificationService->verify($user->applicant, 'email', $request->email, \App\Enums\VerificationMethod::OTP);
                }
            }
        }
