...
class KycController extends Controller
{
    ...
    public function verifyIne(ValidateIneRequest $request): JsonResponse
    {
        ...
        // Toda la orquestación (OCR + INE + RENAPO + persistencia + diffs) vive
        // en el servicio compartido, para reutilizarla desde el panel de staff.
        ...
        $this->logKycAction($request, 'ine_validation', [
            'success' => $result['success'] ?? false,
            'is_valid' => $result['data']['ine_valid'] ?? null,
        ]);
        ...
        return $this->success($result['data'], 'INE verificado');
    }
    ...
    {
        $this->ineVerificationService->updateAndApproveIneDocuments($person, $ocrData, $listValidation);
    }
    ...
    {
        $this->ineVerificationService->saveIdentificationsFromIne($person, $ocrData);
    }
    ...
    {
        $this->ineVerificationService->updateCurpIdentificationStatus($person, $curp, $renapoData);
    }
}
